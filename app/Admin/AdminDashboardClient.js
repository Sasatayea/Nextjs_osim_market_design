...
import {
  ...
  ArrowBack, Category, LocalOffer, AttachMoney
...
const AdminDashboardClient = ({ products }) => {
  ...
  const handleUpdate = async (e) => {
    ...
    setItems((prev) =>
      prev.map((item) => item._id === currentProduct._id ? { ...item, ...rest } : item)
    );
    setIsOpen(false);
  };
  ...
  if (loading) return (
    <Box sx={{ minHeight: "100vh", backgroundColor: "var(--bg)", pt: 8 }}>
  ...
  return (
    <Box sx={{ minHeight: "100vh", backgroundColor: "var(--bg)", py: { xs: 4, md: 8 } }}>
      <Box sx={{ maxWidth: 1200, mx: "auto", px: { xs: 2, md: 4 }, animation: "fadeIn 0.5s ease" }}>
        ...
        <Button
          ...
          onClick={() => router.push("/")}
          sx={{
            color: "var(--text-4)", textTransform: "none", mb: 4, fontWeight: 500, fontSize: "0.85rem",
        ...
        <Box sx={{
          ...
          <Box>
            ...
            <Typography variant="h4" sx={{
              ...
              Admin <em style={{ fontStyle: "italic", color: "var(--gold)" }}>Control</em>
            </Typography>
          </Box>
          ...
          <Button
            ...
            sx={{
              ...
              "&:hover": { filter: "brightness(1.05)", transform: "translateY(-2px)", boxShadow: "var(--shadow-gold-lg)" },
            }}
          >
            Add New Product
          </Button>
        ...
        <Box sx={{
          ...
          {[
            { label: "Products", value: items.length, color: "var(--gold-light)", icon: <Inventory2 /> },
            { label: "Categories", value: Object.keys(groupedProducts).length, color: "var(--blue-light)", icon: <Category /> },
            { label: "Sales Active", value: items.filter(p => p.discount_pct > 0).length, color: "var(--success)", icon: <LocalOffer /> },
            { label: "Market Avg", value: `${Math.round(items.reduce((a, p) => a + (Number(p.sale_price) || 0), 0) / items.length)} EGP`, color: "var(--gold)", icon: <AttachMoney /> },
          ].map((stat) => (
            <Box key={stat.label} sx={{
              p: 3, background: "var(--surface)", borderRadius: "var(--r-lg)",
              border: "1px solid var(--border)", position: "relative",
            }}>
              <Box sx={{ position: "absolute", top: 12, right: 12, color: stat.color, opacity: 0.4 }}>{stat.icon}</Box>
              <Typography sx={{
        ...
        {Object.entries(groupedProducts).map(([category, products]) => (
          <Accordion key={category} defaultExpanded sx={{
            ...
            <AccordionSummary expandIcon={<ExpandMore sx={{ color: "var(--text-4)" }} />} sx={{ px: 3 }}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                ...
                <Typography sx={{ fontWeight: 600, color: "var(--text)", fontSize: "0.9rem" }}>{category}</Typography>
              ...
              {products.map((product) => (
                <Box key={product._id} sx={{
                  ...
                  <Avatar src={product.primary_image_url} alt={product.product_name} variant="rounded" sx={{
                    width: 44, height: 44, borderRadius: "var(--r-sm)", border: "1px solid var(--border)",
                  }} />
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    <Typography sx={{ fontWeight: 600, fontSize: "0.85rem", color: "var(--text-2)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                      {product.product_name}
                    </Typography>
                  </Box>
                  <Box sx={{ display: "flex", gap: 0.5 }}>
                    <AdminIconBtn icon={<Visibility sx={{ fontSize: 16 }} />} onClick={() => handleShow(product._id)} color="var(--blue-light)" />
                    <AdminIconBtn icon={<Edit sx={{ fontSize: 16 }} />} onClick={() => handleEdit(product)} color="var(--gold)" />
                    <AdminIconBtn icon={<Delete sx={{ fontSize: 16 }} />} onClick={() => handleDelete(product._id)} color="var(--error)" />
                  </Box>
                </Box>
              ))}
            </AccordionDetails>
      ...
      <Dialog open={isOpen} onClose={() => setIsOpen(false)} maxWidth="sm" fullWidth PaperProps={{
        ...
        <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid var(--border)", pb: 2 }}>
          <Typography sx={{ fontFamily: "var(--font-display)", fontWeight: 600, color: "var(--text)", fontSize: "1.2rem" }}>Edit Product</Typography>
        ...
        <form onSubmit={handleUpdate}>
          <DialogContent sx={{ py: 3 }}>
            <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
              ...
              <TextField name="sale_price" label="Price" value={form.sale_price || ""} onChange={handleChange} size="small" fullWidth sx={fieldSx} />
            </Box>
          </DialogContent>
          <DialogActions sx={{ px: 3, pb: 3, gap: 1.5 }}>
            <Button onClick={() => setIsOpen(false)} sx={{ color: "var(--text-4)", fontWeight: 600, fontSize: "0.8rem" }}>Cancel</Button>
            <Button type="submit" variant="contained" sx={{
              background: "linear-gradient(135deg, var(--gold-light), var(--gold))", color: "#0a0804",
              borderRadius: "var(--r-sm)", fontWeight: 700, px: 4, py: 1, textTransform: "uppercase", fontSize: "0.75rem",
            }}>Save Changes</Button>
          </DialogActions>
        </form>
      </Dialog>
...
function AdminIconBtn({ icon, onClick, color }) {
  return (
